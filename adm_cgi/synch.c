...
for(z=0; z<3; z++)
{
    ...
    if((client_sock[z] = accept(server_sock,(struct sockaddr *)&client, &client_len)) < 0)
    {
        close(server_sock) ;
        perror("accept");
        exit(-1);
    ...
    for(i=0;i<4;i++)
    {
        ...
        while(filename=readdir(dp))
        {
            ...
            if(0==i)
            {
                ...
                {
                    write(client_sock[0],buff,len);
                    write(client_sock[1],buff,len);
                    write(client_sock[2],buff,len);
    ...
    read(client_sock[0],buff_end,20);                                            //tcp jie shou client end
    read(client_sock[1],buff_end,20);                                            //tcp jie shou client end
    read(client_sock[2],buff_end,20);                                            //tcp jie shou client end
